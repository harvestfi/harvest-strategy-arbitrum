...
contract FluidLendStrategy is BaseUpgradeableStrategy {
    ...
    function _liquidateRewards() internal {
        ...
        for (uint256 i; i < rewardTokens.length; i++) {
            ...
            if (
                balance > rewardBalanceLast[token] ||
                rewardBalanceLast[token] == 0
            ) {
    ...
    function _updateDist(uint256 balance, address token) internal {
        ...
        if (distributionTime[token] > 0) {
            lastRewardTime[token] = lastRewardTime[token] <
                block.timestamp.sub(distributionTime[token])
                ? block.timestamp.sub(distributionTime[token].div(20))
                : lastRewardTime[token];
    ...
    function _getAmt(address token) internal returns (uint256) {
        ...
        uint256 earned = Math.min(
            block.timestamp.sub(lastRewardTime[token]).mul(rewardPerSec[token]),
            balance
        );
