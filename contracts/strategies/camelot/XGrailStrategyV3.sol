...
contract XGrailStrategyV3 is BaseUpgradeableStrategy {
    ...
    function setAllocationTargets(
        ...
        require(addresses.length == weights.length, "len");
        require(addresses.length == poolAddresses.length, "le");
        require(addresses.length == tokenIds.length, "len");
        uint256 totalWeight = 0;
        for (uint256 i; i < addresses.length; i++) {
            if (addresses[i] == dividendsAddress()) {
                require(weights[i] >= 5000, "Div wgt");
            }
        ...
        require(totalWeight == 10000, "Tl wgt");
