...
const Strategy = artifacts.require("CamelotV3NitroStrategyMainnet_rsETH_ETH");
const IXGrail = artifacts.require("IXGrail");
const IVault = artifacts.require("IVault");

const D18 = new BigNumber(Math.pow(10, 18));

//This test was developed at blockNumber 184690650

// Vanilla Mocha test. Increased compatibility with tools that integrate Mocha.
describe("Mainnet Camelot rsETH-ETH V3 HODL in xGRAIL", function() {
  let accounts;
  ...
  async function setupExternalContracts() {
    underlying = await IERC20.at("0xF00Bf7c33eE9106d10661F7ecF5efc73F4C911E1");
  ...
  before(async function() {
    ...
    [controller, vault, strategy, potPool] = await setupCoreProtocol({
      ...
      "liquidation": [
        {"camelotV3": [grail, usdc, weth, rseth]},
      ]
    });
  ...
  describe("Happy path", function() {
    it("Farmer should earn money", async function() {
      ...
      for (let i = 0; i < hours; i++) {
        ...
        hodlPrice = new BigNumber(1900).times(D18);
        underlyingPrice = new BigNumber(4325).times(D18);
