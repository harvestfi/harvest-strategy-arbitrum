...
describe("Mainnet Aave GHO - Rewards", function() {
  ...
  before(async function() {
    ...
    await impersonates([governance, underlyingWhale, rewardSafe]);

    let etherGiver = accounts[9];
    await web3.eth.sendTransaction({ from: etherGiver, to: governance, value: 10e18});
    await web3.eth.sendTransaction({ from: etherGiver, to: rewardSafe, value: 10e18});

    await setupExternalContracts();
    [controller, vault, strategy] = await setupCoreProtocol({
      "existingVaultAddress": "0x85050BEdC80eA28e53dB5F80F165d87F29d2A1bC",
